#include "Ray_coll_app.h"

Ray_coll_app::Ray_coll_app(){}
...
bool Ray_coll_app::Open()
{
	...
	if (this->window->Open())
	{
		...
		Object *obj = CreateObject("resources/weabooMaster.jpg", "resources/vs.txt", "resources/fs.txt",
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				obj = CreateObject("resources/weabooMaster.jpg", "resources/vs.txt", "resources/fs.txt",
										  "resources/cube.obj", true, true, 1);
				obj->Scale(0.4f, 0.4f, 0.4f);
		...
		//FORCE OBJECT
		forceObj = CreateObject("resources/weabooMaster.jpg", "resources/vs.txt", "resources/fs.txt",
								"resources/cube.obj", true, true, 1);
...
void Ray_coll_app::Run()
{
	...
	while (this->window->IsOpen())
	{
		...
		glUseProgram(shader.GetProgram());
		glUniformMatrix4fv(glGetUniformLocation(shader.GetProgram(), "matrix"), 1, GL_TRUE, (projection*view).GetArray());
		glUniform4fv(glGetUniformLocation(shader.GetProgram(), "color"), 1, Vector4D(0,0,0).GetArray());
		glutSolidSphere(0.02f, 10, 10);


		glUniformMatrix4fv(glGetUniformLocation(shader.GetProgram(), "matrix"), 1, GL_TRUE, (projection*view*Matrix4D::SetTranslation(0.5f,1,0)).GetArray());
		glUniform4fv(glGetUniformLocation(shader.GetProgram(), "color"), 1, Vector4D(1,0,0).GetArray());
		glutSolidSphere(0.02f, 10, 10);
		glUniformMatrix4fv(glGetUniformLocation(shader.GetProgram(), "matrix"), 1, GL_TRUE, (projection*view*Matrix4D::SetTranslation(0.5f,-1,0)).GetArray());
		glUniform4fv(glGetUniformLocation(shader.GetProgram(), "color"), 1, Vector4D(0,1,0).GetArray());
		glutSolidSphere(0.02f, 10, 10);
		glUniformMatrix4fv(glGetUniformLocation(shader.GetProgram(), "matrix"), 1, GL_TRUE, (projection*view*Matrix4D::SetTranslation(-1.5f,-1,0)).GetArray());
		glUniform4fv(glGetUniformLocation(shader.GetProgram(), "color"), 1, Vector4D(0,0,1).GetArray());
		glutSolidSphere(0.02f, 10, 10);
		glUniformMatrix4fv(glGetUniformLocation(shader.GetProgram(), "matrix"), 1, GL_TRUE, (projection*view*Matrix4D::SetTranslation(-0.5f,0,-1)).GetArray());
		glUniform4fv(glGetUniformLocation(shader.GetProgram(), "color"), 1, Vector4D(1,1,1).GetArray());
		glutSolidSphere(0.02f, 10, 10);
...
Matrix4D Ray_coll_app::CreateView()
{
...
Matrix4D Ray_coll_app::CreateProjection()
{
...
Object* Ray_coll_app::CreateObject(const char *texture, const char *vs, const char *fs, char *mesh, bool withUV,
...
void Ray_coll_app::DrawForceObject(float dt)
{
	glEnable(GL_CULL_FACE);
